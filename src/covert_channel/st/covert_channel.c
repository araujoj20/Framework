...
void config_DMA_M2M(int size){
    ...
    // static int src, dst;
    HAL_DMA_Start_IT(&hdma, (uint32_t) ADDR_CONTENTION,
...
__attribute__((optimize(0))) void covert_channel(){
    int *ptr_cnt = &htim.Instance->CNT; // (24 bits)
    int *ptr_ctr = &htim.Instance->CR1; // (24 bits)
...
__attribute__((optimize(0))) void DMA_IRQHandler(){
	int *ptr_cnt = &htim.Instance->CNT;
	int *ptr_ctr = &htim.Instance->CR1;
	static int first = 1;
