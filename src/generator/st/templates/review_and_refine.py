...
def validate_and_expand_dmas(project_dmas, board_dma_mapping, board_family, used_dma_names, board_channels_map):
    ...
    for key, cfg in project_dmas.items():
        ...
        # --- Channel count validation against board caps (TIMx_CHy) (wrong) ---
        # if request:
...
def process_timers(project_cfg, board_timer_specs, board_trigger_map, board_caps, board_irq_map):
    ...
    for key, t_cfg in list(project_timers.items()):
        ...
        intr_val_raw = str(t_cfg.get(interrupt_key, "DISABLE")).upper()
        base_has_map = base in board_irq_map
        entry_map = board_irq_map.get(base, {}) if base_has_map else {}
        # Sanitize user value to allowed interrupt modes
        if intr_val_raw not in VALID_INTERRUPT_MODES:
            warn(f"Timer '{key}': invalid interrupt mode '{intr_val_raw}' (forcing DISABLE).")
            intr_val_raw = "DISABLE"

        final_intr_mode = intr_val_raw  # may be adjusted below
        if intr_val_raw != "DISABLE":
            # Handle alias timers first
            if "ALIASES_TO" in entry_map:
                forced_mode = entry_map.get("MODE")
                if forced_mode not in ("BRK","UP","TRG","CC"):
                    error(f"Timer '{key}': invalid MODE '{forced_mode}' in board irq map (alias). Forcing DISABLE.")
                    final_intr_mode = "DISABLE"
                else:
                    final_intr_mode = forced_mode
            elif "SINGLE" in entry_map:
                # Single-vector special case: just treat as ENABLE (no sub-mode)
                if intr_val_raw in ("BRK","UP","TRG","CC"):
                    warn(f"Timer '{key}': composite mode '{intr_val_raw}' ignored (single IRQ vector). Using ENABLE.")
                final_intr_mode = "ENABLE"
            elif all(k in entry_map for k in ("BRK","UP","TRG","CC")):
                # Composite parent timer
                if intr_val_raw == "ENABLE":
                    default_mode = entry_map.get("DEFAULT","UP").upper()
                    if default_mode not in ("BRK","UP","TRG","CC"):
                        default_mode = "UP"
                    final_intr_mode = default_mode
                elif intr_val_raw in ("BRK","UP","TRG","CC"):
                    final_intr_mode = intr_val_raw
                else:
                    final_intr_mode = "UP"
            else:
                # No board composite mapping. Reject composite-specific user request.
                if intr_val_raw in ("BRK","UP","TRG","CC"):
                    warn(f"Timer '{key}': interrupt mode '{intr_val_raw}' ignored (no composite mapping). Using ENABLE.")
                    final_intr_mode = "ENABLE"

        # Persist normalized interrupt mode
        t_cfg[interrupt_key] = final_intr_mode

        # 'irq_handler' now only controls emission (ENABLE/DISABLE). Normalize it.
        irq_key_present = "irq_handler" if "irq_handler" in t_cfg else ("IRQHandler" if "IRQHandler" in t_cfg else None)
        if final_intr_mode == "DISABLE":
            # Interrupt disabled -> remove handler control field (warn if user provided anything)
            if irq_key_present and irq_key_present in t_cfg:
                raw_val = str(t_cfg.get(irq_key_present, "")).upper()
                if raw_val:
                    if raw_val not in ("ENABLE","DISABLE"):
                        warn(f"Timer '{key}': irq_handler='{raw_val}' ignored because interrupt=DISABLE (only ENABLE/DISABLE allowed). Field removed.")
                    else:
                        warn(f"Timer '{key}': irq_handler specified but interrupt=DISABLE; field removed.")
                del t_cfg[irq_key_present]
        else:
            # Ensure handler control exists and valid
            if irq_key_present is None:
                # create canonical key
                t_cfg["irq_handler"] = "ENABLE"
            else:
                val = str(t_cfg.get(irq_key_present, "ENABLE")).upper()
                if val not in ("ENABLE","DISABLE"):
                    warn(f"Timer '{key}': irq_handler value '{val}' invalid (only ENABLE/DISABLE). Using ENABLE.")
                    t_cfg[irq_key_present] = "ENABLE"
...
def refine_project(board_cfg_path, project_cfg_path, output_path):
    ...
    board_irq_map = board_cfg.get("irq_timer_map", {})
    # Não propagar irq_timer_map para o projeto refinado (mantém-se só na board config)
    used_dma_in_timers = process_timers(project_cfg, board_timer_specs, board_trigger_map, board_caps, board_irq_map)
...
def main():
    if len(sys.argv) != 4:
        print("Usage: python review_and_refine.py <board_cfg.json> <project_cfg.json> <output_refined.json>")
        sys.exit(1)
