...
#define USER_CONFIG_FILE "Ideas/interface/user_config.json"
...
int main(int argc, char *argv[]) {
    ...
    cJSON *vendor_json = load_json(input.t_vendor);
    if (!vendor_json) return 1;

    // Carregar a configuração específica da placa
    cJSON *board_json = load_json(input.t_board);
    if (!board_json) {
