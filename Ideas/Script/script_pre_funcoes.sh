...
if [[ " ${steps[@]} " =~ " all " || " ${steps[@]} " =~ " fw_compile " ]]; then
    ...
    mkdir -p $build
    gcc -g3 $interface/interface.c  -o $build/interface     "$fw_dir/cJSON/cJSON.c"     -I"$fw_dir/cJSON"
    gcc -g3 $interface/periph_gen.c -o $build/periph_gen    "$fw_dir/cJSON/cJSON.c"     -I"$fw_dir/cJSON" -D "$board"
fi
