...
// void setup_ns_mpu(void) {
//   uint32_t cache_start = (uint32_t)&__cacheable_text_start__;
//   uint32_t cache_end   = (uint32_t)&__cacheable_text_end__ - 1;

//   ARM_MPU_Disable();

//   ARM_MPU_SetMemAttr(0, ARM_MPU_ATTR_MEMORY_(1, 0, 0, 0)); // non-cacheable

//   MPU->RNR  = 0;
//   MPU->RBAR = 0x08040000;
//   MPU->RLAR = 0x0807FFFF | (0 << MPU_RLAR_AttrIndx_Pos) | MPU_RLAR_EN_Msk;

//   ARM_MPU_SetMemAttr(1, ARM_MPU_ATTR_MEMORY_(1, 1, 1, 1)); // Ncacheable 

//   MPU->RNR  = 1;
//   MPU->RBAR = cache_start;
//   MPU->RLAR = cache_end | (1 << MPU_RLAR_AttrIndx_Pos) | MPU_RLAR_EN_Msk;
