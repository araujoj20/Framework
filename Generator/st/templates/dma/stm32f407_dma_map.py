...
def find_dma_mappings(peripheral_name: str, dma_map: dict):
    results = []
    for dma_name, channels in dma_map.items():
        for channel_name, streams in channels.items():
            for stream_name, peripherals in streams.items():
                if peripherals is None:
                    continue
                if isinstance(peripherals, str):
                    if peripherals == peripheral_name:
                        results.append((dma_name, channel_name, stream_name))
                elif isinstance(peripherals, list):
